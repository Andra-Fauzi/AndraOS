...
// Tokenize dari buffer memori
Token *tokenize_input(char *buf, size_t len) {
    ...
    // Lakukan transformasi yang sama seperti tokenize_file
    canonicalize_newline(p);
    remove_backslash_newline(p);
    convert_universal_chars(p);

    // Buat objek File sementara
    static int file_no;
    File *file = new_file("<input>", file_no + 1, p);

    // Update daftar file (untuk -M & dump dependency)
    input_files = realloc(input_files, sizeof(File *) * (file_no + 2));
    input_files[file_no] = file;
    input_files[file_no + 1] = NULL;
    file_no++;

    // Panggil fungsi tokenize bawaan chibicc
    return tokenize(file);
}