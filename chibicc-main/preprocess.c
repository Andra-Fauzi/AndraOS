...
static Token *include_file(Token *tok, char *path, Token *filename_tok) {
  ...
  if (!tok2)
    error_tok(filename_tok, "%s: cannot open file: %s", path);
  ...
  while (tok->kind != TK_EOF) {
    ...
    if (equal(tok, "include")) {
      ...
      if (filename[0] != '/' && is_dquote) {
        char out[12];
        to_fat_name_83(strdup(start->file->name), out);
        char *path = format("%s/%s", out, filename);
        if (file_exists(path)) {
...
static Token *timestamp_macro(Token *tmpl) {
  // Simplified for kernel integration - no stat/ctime available
...
void init_macros(void) {
  ...
  // Simplified for kernel integration - no time functions available
