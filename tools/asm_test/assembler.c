...
static Label *add_label(const char *name) {
...
static void emit_insn_modrm(uint8_t **buf, int opcode, Operand *op_reg, Operand *op_mem) {
    ...
    // NOTE: op_reg can be NULL for setcc etc where we pass opcode extension manually?
    // Let's assume op_reg is valid for standard instructions.
    // What if reg field represents opcode extension? Caller should set op_reg->reg to it?
...
int process_instruction(const char *line, uint8_t **buf, uint32_t current_addr, int pass) {
    ...
    if (strcasecmp(mnemonic, "nop") == 0) emit_u8(buf, 0x90);
        ...
        emit_u8(buf, 0xE9);
        uint32_t diff = 0;
        Label *l = find_label(op1.label_name);
        ...
        uint32_t diff = 0; Label *l = find_label(op1.label_name);
        ...
        uint32_t diff = 0; Label *l = find_label(op1.label_name); if (pass==2 && l) diff = l->address - (current_addr + 6); emit_u32(buf, diff);
